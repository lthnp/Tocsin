...
@section('content')
    <div class="container">
        ...
        <div class="card-body bg-white rounded shadow-sm mt-3 pb-3 position-relative">
            <form method="post" action="{{ route('dellocation', $location->id) }}">
            ...
            <form method="post" action="{{ route('updatelocation', $location->id) }}" class="bg-white row-cols-2 overflow-auto">
                @csrf
                <div class="col float-left">
                    <div class="form-group mb-4">
                        <p class="font-weight-bold">จังหวัด <span class="text-red">*</span></p>
                        <input id="province" class="form-control rounded-50" type="text" name="province"
                               placeholder="กรอกจังหวัด" value="{{$location->province}}">
                    </div>
                    <div class="form-group mb-4">
                        <p class="font-weight-bold">อำเภอ/เขต <span class="text-red">*</span></p>
                        <input id="amphoe" class="form-control rounded-50" type="text" name="amphur"
                               placeholder="กรอกอำเภอ/เขต" value="{{$location->amphur}}">
                    </div>
                    <div class="form-group mb-4">
                        <p class="font-weight-bold">กรอกตำบล/แขวง <span class="text-red">*</span></p>
                        <input id="district" class="form-control rounded-50" type="text" name="district"
                               placeholder="กรอกตำบล/แขวง" value="{{$location->district}}">
                    </div>
                    <div class="form-group mb-4">
                        <p class="font-weight-bold">รหัสไปรษณีย์ <span class="text-red">*</span></p>
                        <input id="zipcode" class="form-control rounded-50" type="text" name="zipcode"
                               placeholder="กรอกรหัสไปรษณีย์" value="{{$location->zipcode}}">
                    </div>
                    <div class="form-group mb-4">
                        <p class="font-weight-bold">รายละเอียดที่อยู่ <span class="text-red">*</span></p>
                        <textarea class="form-control rounded-nm" name="loc_details" cols="30" rows="5"
                                  placeholder="1-250 ตัวอักษร" maxlength="250">{{$location->loc_details}}</textarea>
                    </div>
                </div>
                <div class="col float-left">
                    <div class="form-group mb-4">
                        <p class="font-weight-bold">หัวข้อเหตุอาชญากรรม <span class="text-red">*</span></p>
                        <input class="form-control rounded-50" type="text" name="news_title"
                               placeholder="1-90 ตัวอักษร" maxlength="90" value="{{$location->news_title}}">
                    </div>
                    <div class="form-group mb-4">
                        <p class="font-weight-bold">รายละเอียดอาชญากรรม <span class="text-red">*</span></p>
                        <textarea class="form-control rounded-nm" name="news_details" cols="30" rows="5"
                                  placeholder="1-250 ตัวอักษร" maxlength="250">{{$location->news_details}}</textarea>
                    </div>
                </div>
                <input type="text" name="news_id" value="{{$news->id}}" hidden>
